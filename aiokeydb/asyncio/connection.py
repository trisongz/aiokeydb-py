...
class BaseParser:
    ...
    def on_connect(self, connection: "AsyncConnection"):
        raise NotImplementedError()
...
class SocketBuffer:
    ...
    async def _read_from_socket(
        ...
        if buf is None or self._stream is None:
            raise KeyDBError("Buffer is closed.")
...
class HiredisParser(BaseParser):
    ...
    def __init__(self, socket_read_size: int):
        if not HIREDIS_AVAILABLE:
            raise KeyDBError("Hiredis is not available.")
...
class ConnectCallbackProtocol(Protocol):
    def __call__(self, connection: "AsyncConnection"):
        ...


class AsyncConnectCallbackProtocol(Protocol):
    async def __call__(self, connection: "AsyncConnection"):
        ...
...
class AsyncConnection:
    ...
    def __init__(
        ...
        *,
        ...
        keydb_connect_func: Optional[ConnectCallbackT] = None,
        ...
        self.ssl_context: Optional[KeyDBSSLContext] = None
    ...
    async def connect(self):
        ...
        try:
            if not self.keydb_connect_func:
                # Use the default on_connect function
                await self.on_connect()
            else:
                # Use the passed function keydb_connect_func
                await self.keydb_connect_func(self) if asyncio.iscoroutinefunction(
                    self.keydb_connect_func
                ) else self.keydb_connect_func(self)
        except KeyDBError:
...
class AsyncSSLConnection(AsyncConnection):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssl_context: KeyDBSSLContext = KeyDBSSLContext(
            keyfile=ssl_keyfile,
...
class AsyncUnixDomainSocketConnection(AsyncConnection):  # lgtm [py/missing-call-to-init]
    def __init__(
        self,
        *,
        path: str = "",
...
_CP = TypeVar("_CP", bound="AsyncConnectionPool")
...
class AsyncConnectionPool:
    ...
    def __init__(
        ...
        **connection_kwargs,
        ...
        self._available_connections: List[AsyncConnection]
