...
from aiokeydb.asyncio.retry import Retry
from aiokeydb.client import (
    AbstractKeyDB,
    EMPTY_RESPONSE,
    NEVER_DECODE,
    CaseInsensitiveDict,
...
_KeyDBT = TypeVar("_KeyDBT", bound="AsyncKeyDB")
_NormalizeKeysT = TypeVar("_NormalizeKeysT", bound=Mapping[ChannelT, object])
if TYPE_CHECKING:
    from aiokeydb.commands.core import Script
...
class AsyncKeyDB(
    AbstractKeyDB, RedisModuleCommands, AsyncCoreCommands, AsyncSentinelCommands
):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # pool, as a similar feature to AsyncConnectionPool's __del__.
    ...
    async def transaction(
        self,
        func: Callable[["AsyncPipeline"], Union[Any, Awaitable[Any]]],
        *watches: KeyT,
    ...
    def lock(
        ...
        lock_class: Optional[Type[AsyncLock]] = None,
        ...
        if lock_class is None: lock_class = AsyncLock
        return lock_class(
            self,
            name,
            timeout=timeout,
...
class AsyncMonitor:
    ...
    async def next_command(self) -> MonitorCommandInfo:
        ...
        # KeyDB escapes double quotes because each piece of the command
...
class PubsubWorkerExceptionHandler(Protocol):
    def __call__(self, e: BaseException, pubsub: AsyncPubSub):
        ...


class AsyncPubsubWorkerExceptionHandler(Protocol):
    async def __call__(self, e: BaseException, pubsub: AsyncPubSub):
        ...
...
class AsyncPipeline(AsyncKeyDB):  # lgtm [py/init-calls-subclass]
    """
    ...
    def execute_command(
        self, *args, **kwargs
    ) -> Union["AsyncPipeline", Awaitable["AsyncPipeline"]]:
    ...
    async def _execute_transaction(  # noqa: C901
        self, connection: AsyncConnection, commands: CommandStackT, raise_on_error
    ):
        pre: CommandT = (("MULTI",), {})
        post: CommandT = (("EXEC",), {})
    ...
    async def _execute_pipeline(
        self, connection: AsyncConnection, commands: CommandStackT, raise_on_error: bool
    ):
    ...
    async def _disconnect_raise_reset(self, conn: AsyncConnection, error: Exception):
        """
