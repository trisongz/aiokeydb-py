...
from aiokeydb.backoff import NoBackoff
...
from aiokeydb.utils import CRYPTOGRAPHY_AVAILABLE, HIREDIS_AVAILABLE, str_if_bytes
...
class BaseParser:
    EXCEPTION_CLASSES = {
        "ERR": {
            ...
            # some KeyDB server versions report invalid command syntax
            # in lowercase
...
class HiredisParser(BaseParser):
    ...
    def __init__(self, socket_read_size):
        if not HIREDIS_AVAILABLE:
            raise KeyDBError("Hiredis is not installed")
        self.socket_read_size = socket_read_size
...
class Connection:
    "Manages TCP communication to and from a KeyDB server"
    ...
    def connect(self):
        "Connects to the KeyDB server if not already connected"
        if self._sock:
            return
        try:
        ...
        self._sock = sock
        try:
            if self.keydb_connect_func is None:
                # Use the default on_connect function
                self.on_connect()
            else:
                # Use the passed function keydb_connect_func
                self.keydb_connect_func(self)
        except KeyDBError:
    ...
    def on_connect(self):
        ...
        if self.username or self.password:
            ...
            try:
                ...
                # a username and password were specified but the KeyDB
    ...
    def disconnect(self, *args):
        "Disconnects from the KeyDB server"
    ...
    def send_packed_command(self, command, check_health=True):
        """Send an already packed command to the KeyDB server"""
        if not self._sock:
            self.connect()
    ...
    def send_command(self, *args, **kwargs):
        """Pack and send a command to the KeyDB server"""
    ...
    def pack_command(self, *args):
        """Pack a series of arguments into the KeyDB protocol"""
        output = []
        # the client might have included 1 or more literal arguments in
        # the command name, e.g., 'CONFIG GET'. The KeyDB server expects these
    ...
    def pack_commands(self, commands):
        """Pack multiple commands into the KeyDB protocol"""
        output = []
        pieces = []
        buffer_length = 0
        buffer_cutoff = self._buffer_cutoff
...
class SSLConnection(Connection):
    """Manages SSL connections to and from the KeyDB server(s).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not ssl_available:
            raise KeyDBError("Python wasn't built with SSL support")
    ...
    def _connect(self):
        ...
        if self.ssl_validate_ocsp is True and CRYPTOGRAPHY_AVAILABLE is False:
            raise KeyDBError("cryptography is not installed.")
...
class ConnectionPool:
    ...
    def from_url(cls, url, **kwargs):
        ...
        found will be used:
            ...
            2. If using the keydb:// or keydbs://, redis:// or rediss:// schemes, the path argument
...
class BlockingConnectionPool(ConnectionPool):
    ...
    Thread-safe blocking connection pool::
        ...
        >>> from redis.client import KeyDB
