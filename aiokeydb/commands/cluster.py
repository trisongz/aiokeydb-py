...
from aiokeydb.compat import Literal
from aiokeydb.crc import key_slot
from aiokeydb.exceptions import KeyDBClusterException, KeyDBError
from aiokeydb.typing import (
    AnyKeyT,
...
class ClusterManagementCommands(ManagementCommands):
    """
    A class for KeyDB Cluster management commands

    The class inherits from KeyDB's core ManagementCommands class and do the
    ...
    def slaveof(self, *args, **kwargs) -> NoReturn:
        ...
        """
        raise KeyDBClusterException("SLAVEOF is not supported in cluster mode")
    ...
    def replicaof(self, *args, **kwargs) -> NoReturn:
        ...
        """
        raise KeyDBClusterException("REPLICAOF is not supported in cluster mode")
    ...
    def swapdb(self, *args, **kwargs) -> NoReturn:
        ...
        """
        raise KeyDBClusterException("SWAPDB is not supported in cluster mode")
    ...
    def cluster_setslot(
        ...
        if state.upper() in ("IMPORTING", "NODE", "MIGRATING"):
            ...
            raise KeyDBError('For "stable" state please use ' "cluster_setslot_stable")
        else:
            raise KeyDBError(f"Invalid slot state: {state}")
    ...
    def cluster_links(self, target_node: "TargetNodesT") -> ResponseT:
        """
        Each node in a KeyDB Cluster maintains a pair of long-lived TCP link with each
...
class ClusterDataAccessCommands(DataAccessCommands):
    """
    A class for KeyDB Cluster Data Access Commands

    The class inherits from KeyDB's core DataAccessCommand class and do the
    required adjustments to work with cluster mode
    """
...
class AsyncKeyDBClusterCommands(
