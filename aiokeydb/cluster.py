...
def get_connection(keydb_node, *args, **options):
    return keydb_node.connection or keydb_node.connection_pool.get_connection(
        args[0], **options
    )
...
class KeyDBCluster(AbstractKeyDBCluster, KeyDBClusterCommands):
    @classmethod
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
         Initialize a new KeyDBCluster client.
         ...
         :require_full_coverage: 'bool'
            ...
            cluster client. If not all slots are covered, KeyDBClusterException
            will be thrown.
        :read_from_replicas: 'bool'
         ...
         :dynamic_startup_nodes: 'bool'
             Set the KeyDBCluster's startup nodes to all of the discovered nodes.
        ...
        :reinitialize_steps: 'int'
         ...
         :**kwargs:
             Extra arguments that will be sent into KeyDB instance when created
             (See Official redis-py doc for supported kwargs
         [https://github.com/andymccurdy/redis-py/blob/master/redis/client.py])
             Some kwargs are not supported and will raise a
             KeyDBClusterException:
                 - db (KeyDB do not support database SELECT in cluster mode)
        """
        ...
        if url is not None:
            ...
            if "path" in url_options:
                raise KeyDBClusterException(
                    "KeyDBCluster does not currently support Unix Domain "
                    "Socket connections"
                )
            ...
            raise KeyDBClusterException(
                "KeyDBCluster requires at least one node to discover the "
                "cluster. Please provide one of the followings:\n"
                "1. host and port, for example:\n"
                " KeyDBCluster(host='localhost', port=6379)\n"
                "2. list of startup nodes, for example:\n"
                " KeyDBCluster(startup_nodes=[ClusterNode('localhost', 6379),"
                " ClusterNode('localhost', 6378)])"
            )
        # Update the connection arguments
        # Whenever a new connection is established, KeyDBCluster's on_connect
        # method should be run
        # If the user passed on_connect function we'll save it and run it
        # inside the KeyDBCluster.on_connect() function
        self.user_on_connect_func = kwargs.pop("keydb_connect_func", None)
        kwargs.update({"keydb_connect_func": self.on_connect})
    ...
    def disconnect_connection_pools(self):
        for node in self.get_nodes():
            if node.keydb_connection:
                try:
                    node.keydb_connection.connection_pool.disconnect()
                except OSError:
    ...
    def get_keydb_connection(self, node):
        if not node.keydb_connection:
            with self._lock:
                if not node.keydb_connection:
                    self.nodes_manager.create_keydb_connections([node])
        return node.keydb_connection
    ...
    def monitor(self, target_node=None):
        ...
        if target_node.keydb_connection is None:
            raise KeyDBClusterException(
                f"Cluster Node {target_node.name} has no keydb_connection"
            )
        return target_node.keydb_connection.monitor()
    ...
    def _get_command_keys(self, *args):
        ...
        """
        keydb_conn = self.get_default_node().keydb_connection
        return self.commands_parser.get_keys(keydb_conn, *args)

    def determine_slot(self, *args):
        """
        Figure out what slot to use based on args.

        Raises a KeyDBClusterException if there's a missing key and we can't
        ...
        if command in ("EVAL", "EVALSHA"):
            ...
            if len(args) <= 2:
                raise KeyDBClusterException(f"Invalid args in command: {args}")
            ...
            if keys is None or len(keys) == 0:
                ...
                if command in ("FCALL", "FCALL_RO"):
                    return random.randrange(0, REDIS_CLUSTER_HASH_SLOTS)
                raise KeyDBClusterException(
                    "No way to dispatch this command to KeyDB Cluster. "
    ...
    def _execute_command(self, target_node, *args, **kwargs):
        ...
        asking = False
        moved = False
        ttl = int(self.KeyDBClusterRequestTTL)
        ...
        while ttl > 0:
            ...
            try:
                ...
                keydb_node = self.get_keydb_connection(target_node)
                connection = get_connection(keydb_node, *args, **kwargs)
                if asking:
                    connection.send_command("ASKING")
                    keydb_node.parse_response(connection, "ASKING", **kwargs)
                    asking = False

                connection.send_command(*args)
                response = keydb_node.parse_response(connection, command, **kwargs)
                ...
                if connection_error_retry_counter < 5:
                    ...
                    target_node.keydb_connection = None
                ...
                if connection is not None:
                    keydb_node.connection_pool.release(connection)
    ...
    def close(self):
        try:
            ...
            # KeyDBCluster's __init__ can fail before nodes_manager is set
            pass
...
class ClusterNode:
    def __init__(self, host, port, server_type=None, keydb_connection=None):
        if host == "localhost":
        ...
        self.keydb_connection = keydb_connection
    ...
    def __repr__(self):
        return (
            ...
            f"keydb_connection={self.keydb_connection}]"
        )
    ...
    def __del__(self):
        if self.keydb_connection is not None:
            self.keydb_connection.close()
...
class NodesManager:
    ...
    def create_keydb_connections(self, nodes):
        ...
        for node in nodes:
            if node.keydb_connection is None:
                node.keydb_connection = self.create_keydb_node(
    ...
    def _get_or_create_cluster_node(self, host, port, role, tmp_nodes_cache):
        ...
        if target_node is None:
            ...
            if target_node is None or target_node.keydb_connection is None:
    ...
    def initialize(self):
        ...
        for startup_node in self.startup_nodes.values():
            try:
                if startup_node.keydb_connection:
                    r = startup_node.keydb_connection
                else:
                    # Create a new KeyDB connection and let KeyDB decode the
                    # responses so we won't need to handle that
                    copy_kwargs = copy.deepcopy(kwargs)
                    copy_kwargs.update({"decode_responses": True, "encoding": "utf-8"})
                    r = self.create_keydb_node(
                        startup_node.host, startup_node.port, **copy_kwargs
                    )
                    self.startup_nodes[startup_node.name].keydb_connection = r
        ...
        if not startup_nodes_reachable:
            raise KeyDBClusterException(
                "KeyDB Cluster cannot be connected. Please provide at least "
                "one reachable node. "
            )

        # Create KeyDB connections to all nodes
        self.create_keydb_connections(list(tmp_nodes_cache.values()))
...
class ClusterPubSub(PubSub):
    ...
    def __init__(self, keydb_cluster, node=None, host=None, port=None, **kwargs):
        """
        ...
        :type keydb_cluster: KeyDBCluster
        :type node: ClusterNode
        :type host: str
        :type port: int
        """
        self.node = None
        self.set_pubsub_node(keydb_cluster, node, host, port)
        connection_pool = (
            None
            if self.node is None
            else keydb_cluster.get_keydb_connection(self.node).connection_pool
        )
        self.cluster = keydb_cluster
        super().__init__(
            **kwargs, connection_pool=connection_pool, encoder=keydb_cluster.encoder
        )
    ...
    def set_pubsub_node(self, cluster, node=None, host=None, port=None):
        ...
        KeyDBClusterException will be thrown if the passed node does not exist
        in the cluster.
        If host is passed without port, or vice versa, a DataError will be
        thrown.
        :type cluster: KeyDBCluster
        :type node: ClusterNode
        :type host: str
        :type port: int
        """
    ...
    def _raise_on_invalid_node(self, keydb_cluster, node, host, port):
        """
        Raise a KeyDBClusterException if the node is None or doesn't exist in
        the cluster.
        """
        if node is None or keydb_cluster.get_node(node_name=node.name) is None:
            raise KeyDBClusterException(
    ...
    def execute_command(self, *args, **kwargs):
        ...
        if self.connection is None:
            if self.connection_pool is None:
                ...
                self.node = node
                keydb_connection = self.cluster.get_keydb_connection(node)
                self.connection_pool = keydb_connection.connection_pool
...
class ClusterPipeline(KeyDBCluster):
    """
    Support for KeyDB pipeline
    in cluster mode
    """
    ...
    def _send_cluster_commands(
        ...
        to false it will raise KeyDBClusterException.
        """
        ...
        for c in attempt:
            ...
            if len(target_nodes) > 1:
                raise KeyDBClusterException(f"Too many targets for command {c.args}")
            ...
            if node_name not in nodes:
                keydb_node = self.get_keydb_connection(node)
                connection = get_connection(keydb_node, c.args)
                nodes[node_name] = NodeCommands(
                    keydb_node.parse_response, keydb_node.connection_pool, connection
                )
    ...
    def eval(self):
        """ """
        raise KeyDBClusterException("method eval() is not implemented")

    def multi(self):
        """ """
        raise KeyDBClusterException("method multi() is not implemented")
    ...
    def _execute_transaction(self, *args, **kwargs):
        """ """
        raise KeyDBClusterException("method _execute_transaction() is not implemented")

    def load_scripts(self):
        """ """
        raise KeyDBClusterException("method load_scripts() is not implemented")

    def watch(self, *names):
        """ """
        raise KeyDBClusterException("method watch() is not implemented")

    def unwatch(self):
        """ """
        raise KeyDBClusterException("method unwatch() is not implemented")
...
class NodeCommands:
    ...
    def write(self):
        """
        Code borrowed from KeyDB so it can be fixed
        """
        connection = self.connection
