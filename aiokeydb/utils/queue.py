...
def get_thread_pool(
    ...
    if _ThreadPool is None:
        if n_workers is None:
            from aiokeydb.utils import get_keydb_settings
            n_workers = get_keydb_settings().worker.threadpool_size
...
def _get_jobkey_func():
    ...
    if _JobKeyFunc is None:
        from aiokeydb.utils import get_keydb_settings
        _JobKeyFunc = _JobKeyMethod[get_keydb_settings().worker.job_key_method]
    return _JobKeyFunc()
