from aiokeydb.v1.exceptions import KeyDBError, ResponseError
...
class CommandsParser:
    ...
    def __init__(self, keydb_connection):
        self.commands = {}
        self.initialize(keydb_connection)
    ...
    def get_keys(self, keydb_conn, *args):
        """
        ...
        if "movablekeys" in command["flags"]:
            keys = self._get_moveable_keys(keydb_conn, *args)
    ...
    def _get_moveable_keys(self, keydb_conn, *args):
        """
        NOTE: Due to a bug in redis<7.0, this function does not work properly
        for EVAL or EVALSHA when the `numkeys` arg is 0.
         - issue: https://github.com/redis/redis/issues/9493
        ...
        try:
            keys = keydb_conn.execute_command("COMMAND GETKEYS", *pieces)
