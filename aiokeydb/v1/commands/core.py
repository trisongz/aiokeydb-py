...
from aiokeydb.v1.exceptions import ConnectionError, DataError, NoScriptError, KeyDBError
...
if TYPE_CHECKING:
    ...
    from aiokeydb.v1.core import KeyDB
...
class ManagementCommands(CommandsProtocol):
    ...
    def sync(self) -> ResponseT:
        ...
        """
        from aiokeydb.v1.client import NEVER_DECODE
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def mget(self, keys: KeysT, *args: EncodableT) -> ResponseT:
        ...
        """
        from aiokeydb.v1.client import EMPTY_RESPONSE
...
class Script:
    ...
    def __call__(
        ...
        from aiokeydb.v1.client import Pipeline
...
class AsyncScript:
    ...
    def __init__(self, registered_client: "AsyncKeyDB", script: ScriptTextT):
        self.registered_client = registered_client
        self.script = script
        # Precalculate and store the SHA1 hex digest of the script.
    ...
    async def __call__(
        ...
        keys: Union[Sequence[KeyT], None] = None,
        args: Union[Iterable[EncodableT], None] = None,
        client: Union["AsyncKeyDB", None] = None,
    ):
        """Execute the script, passing any required ``args``"""
        keys = keys or []
        args = args or []
        ...
        from aiokeydb.v1.asyncio.client import AsyncPipeline

        if isinstance(client, AsyncPipeline):
            # Make sure the pipeline can register the script before executing.
            client.scripts.add(self)
        try:
...
class ScriptCommands(CommandsProtocol):
    ...
    def register_script(self: "KeyDB", script: ScriptTextT) -> Script:
        """
...
class AsyncScriptCommands(ScriptCommands):
    ...
    def register_script(self: "AsyncKeyDB", script: ScriptTextT) -> AsyncScript:
        """
        Register a Lua ``script`` specifying the ``keys`` it will touch.
        Returns a Script object that is callable and hides the complexity of
        deal with scripts, keys, and shas. This is the preferred way to work
