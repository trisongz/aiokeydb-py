...
https://github.com/aio-libs/aioredis-py/blob/master/aioredis/client.py
"""
...
KeyT = _StringLikeT  # Main keydb key space
...
_KeyDBT = TypeVar("_KeyDBT", bound="KeyDB")
...
class KeyDB(Redis):
    ...
    the commands are sent and received to the KeyDB server
    """
    ...
    async def close(self, close_connection_pool: Optional[bool] = None) -> None:
        ...
        let KeyDB.auto_close_connection_pool decide whether to close the connection pool.
        """
    ...
    def bgrewriteaof(self) -> Awaitable:
        """Tell the KeyDB server to rewrite the AOF file from data in memory."""
        return self.execute_command("BGREWRITEAOF")

    def bgsave(self) -> Awaitable:
        """
        Tell the KeyDB server to save its data to disk.  Unlike save(),
    ...
    def client_pause(self, timeout: int) -> Awaitable:
        """
        Suspend all the KeyDB clients for the specified amount of time
    ...
    def info(self, section: Optional[str] = None) -> Awaitable:
        """
        Returns a dictionary containing information about the KeyDB server
        The ``section`` option can be used to select a specific section
        of information
        The section option is not supported by older versions of KeyDB Server,
    ...
    def lastsave(self) -> Awaitable:
        ...
        KeyDB database was saved to disk
        """
    ...
    def migrate(
        ...
        """
        Migrate 1 or more keys from the current KeyDB server to a different
    ...
    def save(self) -> Awaitable:
        """
        Tell the KeyDB server to save its data to disk,
    ...
    def shutdown(self, save: bool = False, nosave: bool = False) -> None:
        """Shutdown the KeyDB server.  If KeyDB has persistence configured,
        ...
        try:
            ...
            return
        raise KeyDBError("SHUTDOWN seems to have failed.")
    ...
    def move(self, name: KeyT, db: int) -> Awaitable:
        """Moves the key ``name`` to a different KeyDB database ``db``"""
    ...
    def scan(
        ...
        ``count`` provides a hint to KeyDB about the number of keys to
            return per batch.
        ``_type`` filters the returned values by a particular Redis type.
            Stock KeyDB instances allow for the following types:
            HASH, LIST, SET, STREAM, STRING, ZSET
            Additionally, KeyDB modules can expose other types as well.
        """
...
class Monitor:
    """
    Monitor is useful for handling the MONITOR command to the keydb server.
    ...
    async def next_command(self) -> MonitorCommandInfo:
        ...
        if self.connection is None:
            raise KeyDBError("Connection already closed.")
        await self.connect()
        response = await self.connection.read_response()
        ...
        if m is None:
            raise KeyDBError("Invalid command received.")
...
class PubSub:
    """
    PubSub provides publish, subscribe and listen support to KeyDB channels.
    ...
    async def run(
        ...
        *,
        ...
        This is the equivalent of :py:meth:`keydb.PubSub.run_in_thread` in
...
class Pipeline(KeyDB):  # lgtm [py/init-calls-subclass]
    """
    Pipelines provide a way to transmit multiple commands to the KeyDB server
    in one transmission.  This is convenient for batch processing, such as
    saving all the values in a list to KeyDB.
    ...
    def multi(self):
        ...
        if self.explicit_transaction:
            raise KeyDBError("Cannot issue nested calls to MULTI")
    ...
    async def watch(self, *names: KeyT):
        ...
        if self.explicit_transaction:
            raise KeyDBError("Cannot issue a WATCH after a MULTI")
...
class Script:
    ...
    def __init__(self, registered_client: KeyDB, script: ScriptTextT):
    ...
    async def __call__(
        ...
        # make sure the KeyDB server knows about the script
...
class BitFieldOperation:
    ...
    def __init__(
        self, client: KeyDB, key: KeyT, default_overflow: Optional[str] = None
    ):
        self.client = client
        self.key = key
